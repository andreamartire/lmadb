...
throws Exception {
	//query
	String query = "SELECT * FROM Bene B WHERE 1=1 ";
	...
	if ( sot != null && !sot.equals("") )
		query = query + " AND upper( sotto_categoria_bene ) LIKE upper('%"+sot+"%')";
...
<%
	...
	<font size=3>Inserisci un bene    	</font><a href="InserisciBene.jsp"><button>Inserisci</button></a><br><br>
	
		<form action="../Login" onclick="return true"><%
			if( request.getParameter("view") == null || request.getParameter("view").equals("tutti") ) {%>
				<input type="radio" name="esito" value="tutti" checked onclick="self.document.location.href='visualizzaBeni.jsp?view=tutti';">Tutti
				<input type="radio" name="esito" value="ricerca" onclick="self.document.location.href='visualizzaBeni.jsp?view=ricerca';">Ricerca<br><br><%
			}
			else {%>
				<input type="radio" name="esito" value="tutti" onclick="self.document.location.href='visualizzaBeni.jsp?view=tutti';">Tutti
				<input type="radio" name="esito" value="ricerca" checked onclick="self.document.location.href='visualizzaBeni.jsp?view=ricerca';">Ricerca<br><br><%
			}%>
		</form><%
	
	if( request.getParameter("view") != null && request.getParameter("view").equals("ricerca") ){%>
		<form action="visualizzaBeni.jsp">
	...
	if(!res.isBeforeFirst()){%>
		...
		<table border="1">
		<tr><th><b><a href="visualizzaBeni.jsp?orderby=numero_inventario_generico">Codice</a></b></th>
		<th><b><a href="visualizzaBeni.jsp?orderby=importo">Importo</a></b></th>
		<th><b><a href="visualizzaBeni.jsp?orderby=garanzia">Garanzia</a></b></th>
		<th><b><a href="visualizzaBeni.jsp?orderby=sotto_categoria_bene">SottoCategoria</a></b></th><th></th><th></th><th></th></tr>
	<%
		while ( res.next() ) {
